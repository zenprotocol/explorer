...
test('voteIntervalsDAL.findAllRecent() (DB)', async function(t) {
  ...
  await wrapTest(
    'Given a few intervals in the future, current is contestant',
    async given => {
  ...
  await wrapTest(
    'Given a few intervals in the future, current is candidate',
    async given => {
      await voteIntervalsDAL.bulkCreate(aFewIntervals);
  ...
  await wrapTest(
    'Given a few intervals in the future, current is 2 contestant',
    async given => {
      await voteIntervalsDAL.bulkCreate(aFewIntervals);
